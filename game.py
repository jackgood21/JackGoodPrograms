...
""" The main idea of this game is to have a player dodge objects falling from above. We plan on having the user input be
 pressing the right and left keys to move the player horizontally.
 -We plan to have enemies be the objects falling toward the player from above.
 -We plan to have a timer as a score (the longer you last, the higher your score)
 -We plan to have a health meter keep track of how many hits the player takes before they lose
 -We plan to have sound effects when a player bumps into an object falling from above
 
 We still need to clean up our start screen, improve character boundaries, add sound. Our program 
 our program also throws an error the second time the player gets hit by a asteroid.
 
 Since the checkpoint we have added a start screen, health bars, time/score counter, and screen boundaries.  
 """
...
start_screen = [
    ...
    gamebox.from_text(400,300, "By Grant Whelan (gaw3ey) and Jack Good (jg8dp)", "Arial", 25, 'white'),
